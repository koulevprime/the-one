...
import routing.chitchat.SocialCharacteristics;
import routing.chitchat.SocialInterests;
import util.IncompleteCSV;
...
public class ChitChatRouter extends ActiveRouter {
	...
	/**
	 * Social interests are the unique keywords and phrases that describe the
	 *  the social profile of the person carrying a mobile device. This is a
	 *  required setting in the configuration file.
	 *  
	 ...
	 *  Likewise, if a mobile node v has social patterns, then these social
	 *  patterns also induce social patterns in u.
	 *  
	...
	/**
	 * Messages forwarded through the ChitChat router require metadata
	 * descriptors. These are keywords that describe the topic of the
	 * message. For instance, a photograph of a cat would have "cat"
	 * as a metadata descriptor.
	 * 
	 * If a collection of messages was constructed for input into the
	 * simulation, then a metadata descriptor file may also be constructed
	 * to describe the content of the messages. This file must be in the format
	 * 
	 *   MESSAGE_ID;METADATA_KEYWORD
	 * 
	 * This is an optional setting in the configuration file. If no file is
	 * specified in the configuration file, then the metadata descriptors of
	 * a particular message are defined as the Social Interests of the 
	 * destination node.
	 */
	...
	public void init(DTNHost host, List<MessageListener> mListeners) {
		...
		// If no routers have been initialized (i.e. the program just started, and this router
		...
		if (noRoutersInitialized()) {
			...
			// If the user specified a social interest file, which associates social interests to an individual router,
			//  then the social interests do not need to be synthesized. However, if no social interest file was
			//  specified, then we need to synthesize the "space" from which social interests are picked.
		...
		if (socialInterestsFileURL == null) {
			...
			i = SocialInterests.buildFromSettingsFile(
				socialInterestsFileURL,
				host.getAddress()
			);
		}
		...
		String socialPatternDecayClassName = socialPatternSettings.getSetting(
				SOCIAL_PATTERN_DECAY_CLASS_S,
				"LogisticDecay"
		);
		characteristics.getSocialPatterns().setDecayModel(
			(AbstractDecay)socialPatternSettings.createIntializedObject(
				CHITCHATDECAY_PACKAGE + socialPatternDecayClassName
		));
	}
	...
	/**
	 * Called when a connection's state changes. If the connection is up, then
	 * a growth event should occur for both nodes.
	...
	public void changedConnection(Connection con) {
		...
		if (con.isUp()) {
			...
			//  at every second, and thus that information will need to be wirelessly communicated
			//  between the two devices at every second. This is not favorable.
			//  Look into alternatives for this chitchatting whenever data analysis is performed.
			//  For instance, when a connection is first established, save the social pattern
			//  details from the other guy and continuously reference this local data to update
			//  the patterns of this node.
			//  Or look into pushing updates.
			//  While two nodes u and v are connected, can the social patterns of v change to include
			...
			//System.out.println("=========================================");
			//System.out.printf("Node #%d:%d patterns\n", getHost().getAddress(), characteristics.getStringInfo());
			...
			//////////////////////////////////System.out.printf("Connection between node #%d and #%d is up\n", getHost().getAddress(), con.getOtherNode(getHost()).getAddress());
			//System.out.printf("Node #%d:%d patterns\n", getHost().getAddress(), characteristics.getStringInfo());
			...
			/////////////////////////////////////System.out.printf("Connection between node #%d and #%d is down\n", getHost().getAddress(), con.getOtherNode(getHost()).getAddress());
			//System.out.printf("Node #%d:%d patterns\n", getHost().getAddress(), characteristics.getStringInfo());
	...
	/**
	 * Tries to start a transfer of message using a connection. If the other node
	 * is a good candidate based on the content strength of both nodes, then the
    ...
    /////////////////////////////////////////////////////////////////System.out.printf("Considering msg %s (#%d->#%d) from node #%d to node #%d\n", m.getId(), m.getFrom().getAddress(), m.getTo().getAddress(), getHost().getAddress(), con.getOtherNode(getHost()).getAddress());
	...
	private boolean otherNodeIsAGoodCandidate(Message m, Connection con) {
		...
		if (other.getAddress() == m.getTo().getAddress()) {
			...
			Collection<String> metadata_descriptors = (Collection<String>) m.getProperty(MESSAGE_METADATA_DESCRIPTORS_PROPERTY_KEY);
			...
			//System.out.println(u_content_strength <= v_content_strength ? "Message will be forwarded" : "No message forwarding");
			//System.out.println("+++++++++++++++++++++++++++++++++++++++++++++++++++++++");
	...
	/**
	 * When a new message is created, we must populate with message with the metadata
	 * descriptors that describe the contents of the message.
	 */
	@Override
	public boolean createNewMessage(Message m) {
		//eta.iteration_marker(SimClock.getIntTime());
		//eta.ping("createNewMessage");
    ////////////////////////////////////System.out.printf("Message #%s has been newly created!\n", m.getId());
	...
	public Message messageTransferred(String id, DTNHost from) {
		...
		//////////////////////////System.out.printf("Msg #%s successfully transferred from node #%d to node #%d\n", id, getHost().getAddress(), from.getAddress());
		/*
		 * For some stupid reason, I cannot get the initTtl value from a message. Instead of
		 * doing the naive thing and creating a getter for that value, I'll do some math
		 * to get it.
		 * 
		 * 0+~--|-------------------|-----------|----------->
		 *   created              now()      initTTL
		 ...
		 *    
		 *      |-------------------------------|
		 *       initTtl = getTtl() + (now() - getCreationTime())
		 *       
		...
		double contentStrength = getCharacteristics().getContentStrength(
				(Collection<String>) m.getProperty(
						MESSAGE_METADATA_DESCRIPTORS_PROPERTY_KEY
		));
		int reducedTtl = (int) Math.floor(initTtl * contentStrength);
		m.setTtl(reducedTtl);
		*/
	 ...
	 * 
	 * If there is no file describing the metadata descriptors, then set
	 *  the metadata descriptors of the message to the social interests of
	 *  the receiver.
	 */
	private Collection<String> loadMessageMetadataDescriptors(Message m) {
		if (messageMetadataFileURL == null) {
			return ((ChitChatRouter) m.getTo().getRouter())
					.getCharacteristics().getSocialInterests().keys();
		}
